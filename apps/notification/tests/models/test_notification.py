...
class GroupWithSameNamespaceTest(TestCase):
    """
    Tests preserves relation between notifications and groups with the same
    namespace.
    """

    def setUp(self):
        self.instance = utils.create_notification(save=False)

    def test_ok_when_no_groups(self):
        """
        Tests success when a notification is created with no group related.
        """
    ...
    def test_error_add_group_diff_namespace(self):
        """
        Test adding group with a different namespace from the notification's
        namespace.
        """
        group = utils.create_group(save=True, ignore_validation=True)

        # Create notification due to n-n relation which has to exists in
        # in persistence before adding group into it.
        self.instance.validate()
        self.instance.save()

        # Group has different namespace from notification instance
        with self.assertRaises(Exception):
            self.instance.groups.add(group)

    def test_success_group_same_namespace(self):
        """
        Test adding group with the same namespace from the notification's
        namespace.
        """
